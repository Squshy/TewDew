...
use crate::tewdew::services::{list, list_with_tasks, retrieve_with_tasks};
...
impl TewDewQuery {
    ...
    #[graphql(guard = "Middleware::Authorized")]
    async fn retrieve_tew_dew<'ctx>(&self, ctx: &Context<'ctx>, id: Uuid) -> ServiceResult<TewDew> {
        let pool = get_pool_from_context(ctx)?;
        let Claims { sub, .. } = get_claims_from_context(ctx)?;

        retrieve_with_tasks(pool, sub, &id).await
    }
}
